...
double dotGridGradient(int ix, int iy, double x, double y)
{
    ...
    //TODO: Compute and return the dot-product
     double distance_y = y - (double)iy;  // coordinate y
     double distance_x = x - (double)ix;  // coordinat x 
    

    // // formular for dot product  Duf=∇f*u
    return distance_x*grid[iy][ix][0]+distance_y*grid[iy][ix][1];
...
Vector4d shoot_ray(const Vector3d &ray_origin, const Vector3d &ray_direction, int max_bounce)
{
    ...
    for (int i = 0; i < light_positions.size(); ++i)
    {
        ...
        const Vector4d specular  = obj_specular_color * pow(std::max(n.dot(N),0.0), obj_specular_exponent);
    ...
    Vector4d colors = shoot_ray(p+1e-4*reflection_direction, reflection_direction, max_bounce);
...
void raytrace_scene()
{
    ...
    double image_x = tan(field_of_view/2)*aspect_ratio*focal_length;
    ...
    for (unsigned i = 0; i < w; ++i)
    {
        for (unsigned j = 0; j < h; ++j)
        {
            ...
            if (is_perspective)
            {
                ...
                ray_origin = camera_position;  // where the shooting arrat comes from
                ray_direction = pixel_center - ray_origin;  // the direction equals to the pixel minus the origian
            }
            else
            {
                // Orthographic camera
